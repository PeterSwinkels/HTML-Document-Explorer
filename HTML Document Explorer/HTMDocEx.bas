...
Dim Level As Long
   ...
   Do Until (Level = 0) And (Frame(Level) >= DocumentO.frames.Length)
      ...
      If NextFrame Is Nothing Then
         DocumentList AddWindowH:=Document.WindowH, AddDocumentO:=Nothing, Refresh:=False, ErrorMessage:=ErrorMessage
      Else
...
Public Function DocumentList(Optional AddWindowH As Long = 0, Optional AddDocumentO As HTMLDocument = Nothing, Optional Index As Long = NONE, Optional Refresh As Boolean = False, Optional ErrorMessage As String = Empty) As HTMLDocumentStr
On Error Resume Next
...
Static DocumentsO() As HTMLDocumentStr
   ...
   ElseIf Not ErrorMessage = Empty Then
      If CheckForError(SafeArrayGetDim(DocumentsO())) = 0 Then
         ReDim DocumentsO(0 To 0) As HTMLDocumentStr
      Else
         ReDim Preserve DocumentsO(0 To UBound(DocumentsO()) + 1) As HTMLDocumentStr
      ...
      DocumentsO(UBound(DocumentsO())).ErrorMessage = ErrorMessage
      DocumentsO(UBound(DocumentsO())).WindowH = AddWindowH
...
Dim Index As Long
   ...
   With Table
      .rows = 1
      .Row = 0
      .Col = 0: .ColAlignment(0) = flexAlignLeftCenter: .Text = "Process:"
      .Col = 1: .ColAlignment(1) = flexAlignLeftCenter: .Text = "Document:"
      Index = 0
      Do
         Document = DocumentList(, , Index:=Index)
         If Document.DocumentO Is Nothing Then
            If Document.ErrorMessage = Empty Then
               Exit Do
            Else
               .rows = .rows + 1
               .Row = .rows - 1
               .CellForeColor = vbRed
               .Col = 0: .Text = GetWindowProcess(Document.WindowH)
               .CellForeColor = vbRed
               .Col = 1: .Text = Document.ErrorMessage
            End If
         Else
            .rows = .rows + 1
            .Row = .rows - 1
            If Not HasParent(Document.DocumentO) Then .Col = 0: .Text = GetWindowProcess(Document.WindowH)
            .Col = 1: .Text = Document.DocumentO.location
         End If
               ...
               For Node = 0 To .Item(Item).Attributes.Length - 1
                  If Not IsNull(.Item(Item).Attributes(Node).nodeValue) Then
                     If IsObject(.Item(Item).Attributes(Node).nodeValue) Then
                        ...
                        End If
                     Else
                        If Not .Item(Item).Attributes(Node).nodeValue = Empty Then
...
On Error Resume Next
   HasParent = Not (DocumentO.frames.Parent.Document Is DocumentO)
End Function
